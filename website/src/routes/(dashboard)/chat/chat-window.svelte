<script lang="ts">
    ...
    import {ActiveSessionState, workshopStore} from "$lib/services";
    import {get} from "svelte/store";

    export let state: ActiveSessionState;
    ...
    $: sessionMessagesStore = state.allMessagesStore;
...
<Card.Root class="flex-col flex h-full border-0">
    <Card.Header class="flex flex-row items-center border-b">
        ...
        <div class="flex items-center space-x-4">
            {#each state.session.agents as spec}
                {@const agent = agents.value.get(spec)}
                {#if agent}
                    <Tooltip.Root>
                        <Tooltip.Trigger>
...
<!--                on:click={() => (open = true)}-->
    ...
    <Card.Footer>
        {#if state.canSendMessage}
            <form
                    on:submit|preventDefault={() => state.sendMessage()}
            ...
            >
                <Input
                        ...
                        bind:value={$input}
                />
                <Button type="submit" size="icon" disabled={!$canSendMessage} on:click>
                    <PaperPlane class="h-4 w-4"/>
                    <span class="sr-only">Send</span>
                </Button>
            </form>
        {:else  }
            <p class="text-muted-foreground">You are not allowed to send messages</p>
        {/if}
...
<!--                            <Avatar.Root>-->
<!--                                &lt;!&ndash;                                <Avatar.Image src={user.avatar} alt="Image"/>&ndash;&gt;-->
<!--                                <Avatar.Fallback>{agent.name[0]}</Avatar.Fallback>-->
<!--                            </Avatar.Root>-->
<!--                            <div class="ml-2">-->
<!--                                <p class="text-sm font-medium leading-none">{agent.name}</p>-->
...
<!--                            &lt;!&ndash;                            <Avatar.Image src={user.avatar}/>&ndash;&gt;-->
