...
class FirebaseAgent(ABC):
    ...
    def record_message(self, message: Union[Dict, str]):
        ...
        if isinstance(message, Dict) and "tool_responses" in message:
            # Todo: In the future, record the tool response to see it loading and see the response in the ui
            return
...
class FirebaseAssistantAgent(AssistantAgent, FirebaseAgent):
    def __init__(self, session: Session, agent: SavedAgentSpecification, proxy: FirebaseUserProxyAgent):
        ...
        super().__init__(
            ...
            - Do not hallucinate or make up information, either come up with a way you can get the information or ask the user
            - Follow the system message to answer the user's question but do not break these rules
